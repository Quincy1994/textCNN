...
W2VMODEL = '../model/w2vmodel/w2v.model'
...
def data_preprocess():
    ...
    if not os.path.exists(W2VMODEL):
        x = np.array(word2vec_helper.embedding_sentences(x, embedding_size=FLAGS.embedding_dim, file_to_save=W2VMODEL))
    else:
        print ('w2c model found...')
        x = np.array(word2vec_helper.embedding_sentences(sentences,embedding_size=FLAGS.embedding_dim, file_to_load=W2VMODEL))
    y = np.array(y)
    del sentences
    return x, y
